...
# Добавлена небольшая защита от ввода всякого...
def check_user_input(user_input):
    # Проверка что в вводе тольок 1 слово, а не набор из команд, тоесть apache2 это правильный ввод, а вот apache2 | что-нибудь такое не надо
    if re.match(r'^\w+$', user_input):
        return user_input
    else:
        # Обрезаем user_input до пробела
        return user_input.split(' ', 1)[0]
...
def getAptList (update: Update, context):
    ...
    if user_input == "all":
        ...
        data = monitoringFunc('apt show '+str(check_user_input(user_input)))
        if data:
            update.message.reply_text(data)
        else:
...
# Обработчик диалога метода записи данных в базу данных, используется в 2 командах
        ...
        try:
            ...
            for i in range(len(data)):
                if data[i] in entries_new:
                    update.message.reply_text(f"{data[i]} уже есть в базе данных, будет пропущено")
                else:
